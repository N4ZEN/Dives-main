...
const Reefhealth = ({ parentCallback, RefHealth }) => {
    ...
    React.useEffect(() => {
        if (reefhealthedit && Object.keys(reefhealthedit).length !== 0) {
            ...
            if (reefhealthedit.Pollutions && Object.keys(reefhealthedit.Pollutions).length !== 0) {
                if (reefhealthedit.Pollutions.Pollutions) {
                setPollutionToggle(reefhealthedit.Pollutions.Pollutions)
                    if (reefhealthedit.Pollutions.Plastic === true) {
                        setPlasticChecked(reefhealthedit.Pollutions.Plastic)
                    }
                    if (reefhealthedit.Pollutions.FishingGear=== true) {
                    ...
                    if (reefhealthedit.Pollutions.Others.Others === true) {
                        ...
                        {bleachingToggle &&
                            <View style={{ borderBottomColor: COLORS.lightGray1, borderBottomWidth: 1 }}>
                                <View style={{ flexDirection: 'row', paddingTop: 8 }}>
                                    ...
                                    <Text style={{ ...styles.sublabelStyles, color: COLORS.black, fontFamily: 'LatoBold' }}>{bleachingLevel.toString()}</Text>
                                </View>
                                <View style={{ flexDirection: 'row' }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
